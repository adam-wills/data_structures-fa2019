...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     /** Helper function for getting square of distance between two points */
     double getDistance(const Point<Dim> &first, const Point<Dim> &second) const;

     /** Recursive helper function used to construct KDTree from vector of Pts */
     void pointsToTree(vector<Point<Dim>> &points, KDTreeNode*& subroot,
                       int r, int l, int curDim);

     /** partition function used in quickSelect implementation */
     int partition(vector<Point<Dim>> &points, int l, int r,
                   int pivotIdx, int curDim);

     /** returns index of kth smallest element in a vector of Points */
     Point<Dim> kthSmallest(vector<Point<Dim>> &points, int l, int r,
                            int k, int curDim);

     /** recursive helper function for finding nearest neighbor */
     void FNN_helper(const Point<Dim> &query, KDTreeNode* subroot,
                     KDTreeNode*& curBestNode, const int curDim) const;

    /** helper function for above helper function */
     void FNN_update(const Point<Dim>& query, KDTreeNode* subroot,
                     KDTreeNode*& curBestNode) const;

     /** copy helper function to assist with rule of 3 implementation */
     void copy_(const KDTreeNode* otherRoot, KDTreeNode*& subroot);

     /** clear helper function to assist with rule of 3 implementation */
