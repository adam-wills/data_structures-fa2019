...
int LPHashTable<K, V>::findIndex(const K& key) const
{
     ...
     */
     ...
     while (should_probe[idx]) {
       if (table[idx] && table[idx]->first == key) {return idx;}
       idx = (idx+1)%size;
     ...
     */
     ...
     for (size_t i = 0; i < size; ++i) {
       if (table[i]) {
         ...
         while(newTable[newIdx]) {newIdx = (newIdx+1)%newSize;}
         newTable[newIdx] = table[i];//new std::pair<K,V>(table[i]->second);
