                            ...
                            queue<TreeNode*>& mergeQueue)
{
  ...
  if (singleQueue.size() + mergeQueue.size() > 1) {
    ...
    if (!singleQueue.empty() && !mergeQueue.empty()) {
      TreeNode* item1 = singleQueue.front();
      TreeNode* item2 = mergeQueue.front();

      if ((item1->freq).getFrequency() < (item2->freq).getFrequency()
        ||(item1->freq).getFrequency() == (item2->freq).getFrequency()) {
          ret = item1;
          singleQueue.pop();
      }
      else {
