...
void Allocator::copyAllocator(const Allocator& other)
{
  roomCount = other.roomCount;
  studentCount = other.studentCount;
  totalCapacity = other.totalCapacity;

  alpha = new Letter[26]; // deep copy
  for (int i = 0; i < 26; i++) {alpha[i] = other.alpha[i];}

  rooms = new Room[roomCount]; // deeeeep copy
  for (int i = 0; i < roomCount; i++) {rooms[i] = other.rooms[i];}
}
