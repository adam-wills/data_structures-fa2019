...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if (elements.at(0) > val || val == elements.at(0)) {return 0;}
    for (size_t i = 1; i < elements.size(); ++i) {
      if (val == elements.at(i) || (val < elements.at(i) && val > elements.at(i-1))) {
        return i;
      }
    }
...
template <class T, class C>
size_t binary_idx(const std::vector<T>& elements, size_t l, size_t r, const C& val);

template <class T, class C>
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
  if (elements.empty()) {return 0;}
  return binary_idx(elements,0,elements.size()-1,val);
}
...
{
  if (l == r) {
    if (val == elements.at(r) || val < elements.at(r)) {return r;}
    else {return r+1;}
  }
  size_t mid_idx = (l+r)/2;
  if (val == elements.at(mid_idx)) {return mid_idx;}
  if (val > elements.at(mid_idx) && val < elements.at(mid_idx+1)) {
    return mid_idx+1;
  }
  else if (val < elements.at(mid_idx)) {
    return binary_idx(elements,l,mid_idx,val);
  }
  else {return binary_idx(elements,mid_idx+1,r,val);}
}
